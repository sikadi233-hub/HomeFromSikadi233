...
public class TpaCommand implements CommandExecutor {
    ...
    public TpaCommand(HomeFromSikadi233 plugin) { this.plugin = plugin; }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player)) { sender.sendMessage("只有玩家可用！"); return true; }
        Player player = (Player) sender;
        if (args.length < 1) { player.sendMessage(ChatColor.RED + "用法：/tpa <玩家名>"); return true; }
        ...
        plugin.addTpaRequest(target.getUniqueId(),
                new HomeFromSikadi233.TpaRequest(player, target, HomeFromSikadi233.TpaType.TPA, System.currentTimeMillis()));

        int timeout = plugin.getTpaTimeout();
        player.sendMessage(ChatColor.GREEN + "📨 已向 " + ChatColor.GOLD + target.getName() + ChatColor.GREEN + " 发送TPA请求！(" + timeout + "秒)");

        target.sendMessage("");
        target.spigot().sendMessage(new ComponentBuilder("📨 ")
                .color(net.md_5.bungee.api.ChatColor.GREEN)
                .append(player.getName()).color(net.md_5.bungee.api.ChatColor.GOLD)
                .append(" 请求传送到你这里！").color(net.md_5.bungee.api.ChatColor.GREEN).create());
